...
- Config injection is **fail-closed when network restriction is on** (the contract is load-bearing: it is where the result-file and callback rules live) and warn-only when `JIFFY_SANDBOX_NETWORK_RESTRICTED=false` (debugging mode). A missing root `opencode.json` follows the same rule.
