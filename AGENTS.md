...
4. `build_agent_instructions` (in `jobs/execution/agent.py`) has the agent compose a **human-readable markdown comment body** (`Task #N: ✅ Jiffy completed this task.` with `**Summary:**`, `**Branch:**`, `**Pull Request:**`, `### Technical Report`; failure format uses `❌` + `**Reason:**`). Omit empty lines/sections.
...
- **The task travels by file, never by argv.** `opencode run` takes its prompt from argv only — no stdin, no prompt-file flag — and the kernel caps a single argv entry at 128 KiB (`MAX_ARG_STRLEN`), so the request itself cannot ride in the prompt. `build_task_document()` writes the complete task to **`/tmp/jiffy_task.json`** (`TASK_JSON_PATH`): `issue_text` holds the whole verbatim thread plus its byte count and sha256, alongside `task_id`, `turns`, `repo_url`, `workspace`, `result_path`. That file is the single source of truth, and it carries **no credentials** — the repo token is container env, the callback secret lives in the callback wrapper's config.
- **The request is ALWAYS inlined in the prompt**, between `ISSUE_BEGIN_MARKER`/`ISSUE_END_MARKER` — never replaced by a pointer. `_issue_section()` inlines it whole up to `MAX_INLINE_ISSUE_TEXT_BYTES` (80 KiB, leaving a wide margin under the 128 KiB argv cap); above that it keeps the head (`ELISION_HEAD_SHARE`, 60%) and the tail and marks the omitted middle, so the original request *and* the latest comments are both in front of the agent, with the complete copy in the task file. Two reported failures came from breaking this rule: an empty fenced region produced "the message appears to be cut off", and a bare "read this file" pointer produced "What would you like me to work on?" — a small model does not go and read a file it was merely told about.
- **The prompt starts with the task, not with boilerplate.** The rendered prompt opens with `# YOUR TASK — START WORKING ON THIS NOW` and the fenced request, and only then `# HOW TO CARRY IT OUT` with the standing procedure. A small model (the deployments run cheap ones) that meets a wall of preamble first tends to answer conversationally instead of working.
- **Delivery to the agent's argv is verified, not assumed.** What reaches `opencode` is a command substitution in a login shell, not the file itself. `verify_prompt_reaches_argv()` runs `printf %s "$(cat …)" | wc -c` through that same `bash -l -c` path before the agent starts and fails the task if the byte count does not match what was staged (allowing for the trailing newlines command substitution strips). The run command also echoes `[jiffy] handing N chars of prompt to opencode` into the container log, so `docker logs` shows what the agent was handed. An agent given an empty prompt answers "What would you like me to work on?", which is otherwise indistinguishable from a dozen unrelated faults.
- Both staged files go up as tar uploads (`put_archive`, never echoed through a shell — a shell command is itself an argv entry) and **every staged file is verified after upload** (`stat -c %s` read back, compared to what was sent). A truncated or missing file fails the task with an accurate reason. `_log_prompt_delivery()` logs the issue-text byte count, its sha256, the prompt size, and whether the request went inline or by reference — one worker-log line settles what the agent received.
