...
A question costs a human round trip and stalls the task, so the instructions treat asking as a **last resort**, not a safety net: search the repo first, do every part that doesn't depend on the answer, take the reasonable default and state the assumption in the summary/report, and check the answer would actually change the code. Only a genuinely blocking case — destructive/unsafe either way, a credential it cannot obtain, or two readings that lead to different work with nothing in the repo favouring either — earns a question. But a blocking question must never be guessed at or left in the logs: nothing about a run is interactive, so a question the agent cannot post is a question nobody sees.

- **The issue text is fenced** by `ISSUE_BEGIN_MARKER`/`ISSUE_END_MARKER` (`<<<JIFFY-ISSUE-BEGIN>>>` / `<<<JIFFY-ISSUE-END>>>`, `jobs/execution/agent.py`). Seeing the end marker is the agent's proof it received the whole thread; the instructions forbid the one question never worth asking — "the text reached me incomplete, please re-send it" — and tell the agent to read `/tmp/jiffy_instructions.txt` (always complete) if the end marker is missing, and to treat a genuinely short request as the whole request. `_extract_issue_text()` logs a warning when the extracted text is blank, and `execute_task` logs the instruction size at handoff, so a truncation shows up as one log line instead of as an agent asking what the task was.
- The agent writes `status: "question"` plus a `question` field in `.jiffy_result.json`, and delivers it through the same one-shot callback with a `[JIFFY:QUESTION] Task #N: ❓ Jiffy has a question before continuing.` body (`**Question:**`, optional `**Branch:**` / `**Progress so far:**`, and a closing "reply on this issue" note). The leading `QUESTION_TAG` (`apps/ingestion/callback.py`) is what marks the comment as a question rather than a result — for a human scanning the thread, for the edge, and for the next run reading its own earlier turn. Success and failure bodies never carry it.
...
- **The prompt is a fixed ~15 KB regardless of thread size.** `_issue_section()` reproduces the request between `ISSUE_BEGIN_MARKER`/`ISSUE_END_MARKER` while it is under `MAX_INLINE_ISSUE_TEXT_BYTES` (48 KiB) — small models follow a task far better when it is in front of them. Above that it renders **no markers at all** and instead states plainly that the text is not in the message, names `/tmp/jiffy_task.json`, and gives the exact `python3 -c` command to print it. What it never emits is an empty or half-filled fenced region: that is what made the agent answer "the message appears to be cut off" and ask for the task to be re-sent. Both renderings forbid that question outright.
