...
- **`opencode run` takes its prompt from argv only** — no stdin, no prompt-file flag — and the kernel caps a single argv entry at 128 KiB (`MAX_ARG_STRLEN`). So **two** copies are staged, both as tar uploads (`put_archive`, never echoed through a shell — a shell command is itself an argv entry): `/tmp/jiffy_instructions.txt` always holds the **complete** instructions, and `/tmp/jiffy_prompt.txt` holds the copy the shell substitutes into argv. Under `MAX_INLINE_INSTRUCTIONS_BYTES` (96 KiB) they are identical. Above it, `build_inline_instructions()` re-renders with the **issue text** trimmed to fit while the whole contract around it is preserved, and the trimmed region carries a pointer to the full file. The agent therefore always gets a valid, self-contained prompt containing the start of the real request — never a bare "go read this file" it has to act on faith. (It used to get exactly that, and would then report it had no task.)
- **Every staged file is verified after upload** (`stat -c %s` read back, compared to what was sent). A truncated or missing prompt fails the task with an accurate reason instead of surfacing later as the agent asking what the task was. `_log_prompt_delivery()` logs the byte counts and a sha256 prefix at handoff, so one worker-log line settles whether the agent received the whole request.
