...
1. **The sandbox never talks to the Gateway.** The Gateway reads the run's outcome off the container's filesystem (`.jiffy_result.json` via `docker exec`) and polls the exec state over the Docker API — there is no sandbox→Gateway HTTP at all. The sandbox's only outbound call is the callback, and it goes to the **git provider's** comment API (`callback.url`, e.g. `https://api.github.com/repos/.../issues/N/comments`), not to the Gateway.
2. **That one call is delivered by deterministic code, not by the agent.** `stage_callback_wrapper()` (`jobs/execution/container.py`) uploads `jobs/execution/sandbox_scripts/jiffy_callback.py` to `/tmp/jiffy_callback.py` plus a `/tmp/jiffy_callback.json` config built by `build_sandbox_callback_config()`. The agent composes the markdown body, writes it to a file, and runs `python3 /tmp/jiffy_callback.py --body-file <path>` **once**; the script owns the HTTP call, the retries, and the `{attempted, succeeded, error}` object the agent copies verbatim into its result. The prompt carries no retry policy — it forbids hand-rolled requests, curl, and retry loops. The script is stdlib-only (the sandbox Python has no third-party packages) and staged per run rather than baked into the image, so it stays in lockstep with the Gateway without an image rebuild.
3. **Retry policy lives in `jobs/callback_retry.py`** and is shared by the wrapper and the Gateway's own sender, so both behave identically: `MAX_ATTEMPTS = 3` (matching `execute_task`'s `max_retries`), exponential backoff from the sender's existing 2s (2s then 4s), `TIMEOUT_SECONDS = 30` per request. Retries cover transport errors, timeouts, 5xx, and 408/429; **every other 4xx fails immediately** — a rejected secret or a deleted issue answers the same way however often it is asked. Nothing waits unbounded: attempts and per-request timeouts are both capped, so an unreachable endpoint ends as a logged failure and the Gateway-side run timeout stays the final safety net. Every attempt and the final outcome are logged (worker log for the Gateway, container stderr → `docker logs` for the wrapper).
4. The **system prompt** (`build_system_prompt` in `jobs/execution/agent.py`) has the agent compose a **human-readable markdown comment body** (`Task #N: ✅ Jiffy completed this task.` with `**Summary:**`, `**Branch:**`, `**Pull Request:**`, `### Technical Report`; failure format uses `❌` + `**Reason:**`). Omit empty lines/sections.
5. Wire format is provider-specific via `jobs/callback_specs.py`: **GitHub** = JSON `{"body": "<markdown>"}` with `Accept: application/vnd.github+json` and `X-GitHub-Api-Version: 2026-03-10` (the callback URL is the Issue Comments API); **GitLab/Gitea** = plain `text/plain; charset=utf-8`. Add a new provider by adding a spec entry, not pipeline code.
6. `execute_task` checks the agent result's `callback.attempted/succeeded`; if the agent did not deliver, the Gateway falls back via `send_fallback_callback` (same spec, same `jobs/callback_retry.py` policy — task stays `done`/`failed` even if all attempts fail). `send_callback` is the older path, used only in tests.
7. `technical_report` is a structured markdown report (`## What was done`, `## Technology / approach chosen`, `## Reasoning`, `## Setup / installation instructions`, `## Known limitations / follow-ups`).

## Agent questions — forbidden in the prompt

Nothing about a run is interactive: no human is watching, so a question is simply a run that did nothing. The prompt therefore has **no** question mechanism at all. Its `## When Something Is Unclear` section tells the agent to never ask, to resolve ambiguity from the repository, to take the most reasonable reading, to write the assumption into `summary`/`technical_report`, and to complete every part it does understand. It explicitly forbids "what would you like me to work on?" and any request to have the task re-sent. The only non-finished outcome offered is an honest `"failed"` result, for things it genuinely cannot do.

The Gateway side is kept as a defensive path, not as something the prompt invites:

- `VALID_AGENT_STATUSES` still accepts `"question"`, and `read_agent_result()` still parses a `question` field (downgrading a `"question"` with no text to `failed`). If a model produces one anyway, it is relayed rather than dropped.
- `execute_task` routes it to `_ask_task()`: status `needs_input`, the question stored on the row, and the text posted to the issue through the usual agent-first/Gateway-fallback callback with the `QUESTION_TAG` (`[JIFFY:QUESTION]`) prefix that marks a comment as a question rather than a result.
- Success and failure bodies never carry that tag.

## Agent instruction contract

- `build_agent_instructions(payload)` builds the per-task **user prompt**: the verbatim issue text (from `turns` if present, else `text`), fenced between markers, plus a "Where to Start" pointer. `build_system_prompt(payload)` builds the agent-agnostic **system prompt**: workspace at `/workspace`, and the full self-provisioning workflow (analyze → install missing tools → implement → verify → commit/push on a new branch → open PR **only if asked** → mention the review bot **only if asked** → attempt callback).
...
- One generic image (default tag `jiffy-sandbox:1.3.0`, from `SANDBOX_IMAGE`), built from `docker/sandbox/Dockerfile` on demand by `ensure_sandbox_image()` if missing. Bundles nvm/uv/gvm, node, python, go, gh/glab, git, curl, build-essential, iptables, pnpm, and the `opencode` CLI — pinned to **1.18.30** via the `OPENCODE_VERSION` build arg, because 1.18.31+ rejects Zen's `-free` models with a 403 the Gateway only ever sees as `UnknownError`. Bump the pin and the image tag together, or `ensure_sandbox_image()` keeps the old image. Everything else is installed by the agent at runtime — there is no per-language image selection and no pre-container language check.
...
- The agent runs as `opencode run --auto "$(cat /tmp/jiffy_prompt.txt)"` inside `bash -l -c` (login shell so `/etc/profile.d` version managers load), workdir `/workspace`. Agent stdout is redirected to the container's main stdout for live `docker logs`.
- **The prompts travel by file, never by argv.** `build_task_document()` writes the complete task to **`/tmp/jiffy_task.json`** (`TASK_JSON_PATH`): `issue_text` holds the whole verbatim thread plus its byte count and sha256, alongside `task_id`, `turns`, `repo_url`, `workspace`, `result_path`. That file is the single source of truth, and it carries **no credentials** — the repo token is container env, the callback secret lives in the callback wrapper's config.
- **The prompt is split in two.** `build_agent_instructions()` (agent.py) builds the **user prompt** — the request for this run, fenced by `ISSUE_BEGIN_MARKER`/`ISSUE_END_MARKER`, plus a "Where to Start" block when the issue text carries a "Suggested codebase areas" section. `build_system_prompt()` builds the **system prompt** — the standing contract (working directory, resource limits, no-questions rule, callback delivery, result file), agent-agnostic and issue-free. The request is ALWAYS inlined in the user prompt — never replaced by a pointer; an empty fenced region produced "the message appears to be cut off" and a bare "read this file" pointer produced "What would you like me to work on?". `_issue_section()` inlines it whole up to `MAX_INLINE_ISSUE_TEXT_BYTES` (80 KiB, a context budget, not a delivery limit); above that it keeps the head (`ELISION_HEAD_SHARE`, 60%) and the tail and marks the omitted middle, with the complete copy in the task file.
- **Delivery is stdin, never argv.** `opencode run` merges piped stdin into its message (`resolveRunInput` in the CLI source), so `build_agent_command()` runs `opencode run --auto < /tmp/jiffy_prompt.txt`: a single argv entry is capped at 128 KiB by the kernel (`MAX_ARG_STRLEN` — exit 126 "Argument list too long"), while stdin is bounded only by disk. The system prompt is staged at `/tmp/jiffy_system_prompt.txt` (`SYSTEM_PROMPT_PATH`) and loaded by the agent itself: `_inject_opencode_config()` merges that path into the `instructions` list of the injected `opencode.json` (existing entries kept, ours first, no duplicates).
- **The user prompt starts with the task, not with boilerplate.** It opens with `# YOUR TASK — START WORKING ON THIS NOW` and the fenced request, and ends by pointing at the system prompt file for the standing procedure. A small model (the deployments run cheap ones) that meets a wall of preamble first tends to answer conversationally instead of working.
- **Delivery is verified, not assumed.** `verify_prompt_delivery()` reads the staged user prompt back through the same `bash -l -c` path and fails the task if the byte count does not match what was staged (stdin delivery is verbatim — no newline stripping to allow for). The run command also echoes `[jiffy] handing N bytes of prompt to opencode via stdin` into the container log, so `docker logs` shows what the agent was handed. An agent given an empty prompt answers "What would you like me to work on?", which is otherwise indistinguishable from a dozen unrelated faults.
- Config injection is **fail-closed when network restriction is on** (the contract is load-bearing: it is where the result-file and callback rules live) and warn-only when `JIFFY_SANDBOX_NETWORK_RESTRICTED=false` (debugging mode). A missing root `opencode.json` follows the same rule, and so does an empty or unparseable one — it would otherwise be merged as `{}`, silently dropping `model` and `plugin` and leaving the agent on whatever default its credentials resolve to.
- Staged files go up as tar uploads (`put_archive`, never echoed through a shell — a shell command is itself an argv entry) and **every staged file is verified after upload** (`stat -c %s` read back, compared to what was sent). A truncated or missing file fails the task with an accurate reason. `_log_prompt_delivery()` logs the issue-text byte count, its sha256, the user-prompt size, the stdin delivery, and whether the request was inlined in full or elided — one worker-log line settles what the agent received.
- **An empty issue text never reaches the agent**: `build_agent_instructions()` raises `AgentError`, which `execute_task` turns into a normal failed task with a clear message on the issue.
- **A run that ends without `/workspace/.jiffy_result.json`** logs the tail of the agent's own output (`_log_agent_output_tail`, 60 lines / 4000 chars, worker log only — never posted to the issue). Without it the container is discarded and the only record is a generic "did not produce the required output contract". The system prompt makes the result file numbered step 9 and opens by telling the agent that nothing it says in chat reaches anyone.
